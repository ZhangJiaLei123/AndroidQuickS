...
public abstract class QPermissionActivity
    ...
    protected String[] permissions = new String[]{"android.permission.READ_EXTERNAL_STORAGE"};

    protected void onResume() {
        super.onResume();
        doCheckPermission((Activity) this);
        if (FAL_PERMISSIONS_CHAEK_WINDOWS) {
            doWindowsCheck((Context) this);
        }
        if (FAL_PERMISSIONS_CHAEK_NOTIFICATION) {
            doNotificationCheck((Activity) this);
        }
        if (FAL_PERMISSIONS_CHAEK_SYSTEM) {
            doSystemCheck((Activity) this);
        }
        if (FAL_PERMISSIONS_CHAEK_APP_AUTO_RUN) {
            jumpStartInterface((Context) this);
        }
    }


    protected void setPermissions(@NonNull String... permissions) {
        ...
        AndPermission.with(activity)
                ...
                .onGranted(new Action() {
                    public void onAction(Object data) {
                        if (QPermissionActivity.this.callBack != null)
                            QPermissionActivity.this.callBack.onCheckPermissionResult(true);
            ...
            public void onAction(Object data) {
                if (QPermissionActivity.this.callBack != null)
                    QPermissionActivity.this.callBack.onCheckPermissionResult(false);
